...
class net(object):
    def __init__(self):
        ...
        def split_channel(patch):  # axis=2给他unstack，然后把这些重新stack 就完成了channel维度的拆分重组
            channel_list = tf.unstack(patch, axis=2)# 32*4*4*128
            return tf.stack(channel_list, axis=0)
        ...
        def deal_per_patch(patch):  # [256 ,4*4, channel] 如何让他变成以channel为导向的呢？
            ...
            tire1 = split_channel(tire1)  # 这里之前回来的等于是一个数组 128个4*4
            tire2 = split_channel(tire2)  #
            tire3 = split_channel(tire3)  #
        ...
        def conv_net():
            with arg_scope([layers.conv2d], padding='SAME',
                ...
                # 或者权重能否加在内部？
                for i in range(batch_size_train):#net_list[i] = 256*4*4*128
