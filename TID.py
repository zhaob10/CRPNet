...
import loader_patch一整张_tid as loader
...
# 顺序
ORDER = config.ORDER
ORDER_csiq = config.ORDER_csiq
ORDER_tid = config.ORDER_tid
...
train_data_dir_tid = config.train_data_dir_tid
train_data_dir_csiq = config.train_data_dir_csiq
test_data_dir = config.test_data_dir
test_data_dir_tid = config.test_data_dir_tid
test_data_dir_csiq = config.test_data_dir_csiq
# 图像大小
...
# 注意这里的改动
input = tf.placeholder(tf.float32, [batch_size_train * 128, 32, 32, 1])
...
init_learning_rate = 0.005  # 这里记得改
...
class net(object):
    def __init__(self):
        ...
        def deal_per_patch(patch):  # [256 ,4*4, channel] 如何让他变成以channel为导向的呢？
            tire1, tire2, tire3 = tf.split(patch, [32, 32, 64], axis=0)  # [64,4*4,64] 分成四份
...
data_train = loader.get_traindata(train_data_dir_tid, ORDER_tid, 20, 5, batch_size_train,capacity_train, min_after_dequeue_train)
data_test = loader.get_testdata(test_data_dir_tid, ORDER_tid, 20, 5, 600, 600,0)
    ...
    for k in range(batch):
        for i in range(16):  # 行
            for j in range(16):  # 列
                ...
                a = a + 1
    img_res = np.zeros([128 * batch, 32, 32, 1])
    q = 0
...
with tf.Session(config=configer) as sess:
    ...
    #算分
    def calScore1(batch, img_t_total,dmos_t_total,weight_t_total):
        pre_total = []
        dmos_total = []
        for j in range(int(batch/batch_size_train)):  #
    ...
    def calScore2(batch, img_t_total,dmos_t_total,weight_t_total):
        pre_total = []
        dmos_total = []
        for j in range(int(batch/batch_size_train)):  #
            img_t = img_t_total[j * batch_size_train:(j + 1) * batch_size_train, :, :, :]
        ...
        for i in range(44001):
            ...
            if i<42000:
                ...
                if (i % 200 == 0 ):
                    ...
                    #输出是  [batchSize*128,1]
                    statics = calScore1(600,img_t_total, dmos_t_total, weight_t_total)
                    if (i >= 20000 ):
                        if (statics[1] < los_tmp and statics[0] > 0.97):
                            saver.save(sess, r'F:\zch\plan2020\logs\2step_tid\step1\one_init_los/train.ckpt')
                            los_tmp = statics[1]

                        if (statics[0] > src_tmp):
                            saver.save(sess, r'F:\zch\plan2020\logs\2step_tid\step1\one_init_src/train.ckpt')
                            src_tmp = statics[0]
            else:
                ...
                if (i % 200 == 0 ):
                    ...
                    dmos_t_total = dmos_t_total.reshape([-1, 1])
                    #输出是  [batchSize*128,1]
                    statics = calScore2(600, img_t_total, dmos_t_total, weight_t_total)
                    if (i >= 20000 ):
                        if (statics[1] < los_tmp and statics[0] > 0.97):
                            saver.save(sess, r'F:\zch\plan2020\logs\2step_tid\step2/one_init_los/train.ckpt')
                            los_tmp = statics[1]

                        if (statics[0] > src_tmp):
                            saver.save(sess, r'F:\zch\plan2020\logs\2step_tid\step2\one_init_src/train.ckpt')
                            src_tmp = statics[0]
